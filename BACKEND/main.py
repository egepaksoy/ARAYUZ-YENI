...
# --- Video Management (Çalışan UDP Koduna Göre Uyarlandı) ---
class VideoManager:
    def __init__(self, udp_ip="0.0.0.0", udp_port=5000):
        self.udp_ip = udp_ip
    ...
    def start(self):
        ...
        try:
            ...
            self.sock.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1) # Portun takılı kalmasını önler
            self.sock.bind((self.udp_ip, self.udp_port))
            self.sock.settimeout(1.0) 
            print(f"[Video] UDP Server {self.udp_ip}:{self.udp_port} üzerinde dinleniyor...")
        except Exception as e:
            print(f"[Video] UDP Soket hatası: {e}")
            self.is_running = False
            return
    ...
    def stop(self):
        ...
        if self.sock:
            self.sock.close()
        print("[Video] UDP Camera stopped")
    ...
    def _capture_loop(self):
        ...
        while self.is_running:
            try:
                ...
                if self.is_running:
                    print(f"[Video] Veri alma hatası: {e}")
                break
            ...
            if frame_number != current_frame:
                if buffer:
                    ...
                    if frame is not None:
                        # Frontend'e yollamak için JPEG formatına kodla
                        ret, jpeg_buffer = cv2.imencode('.jpg', frame, [int(cv2.IMWRITE_JPEG_QUALITY), 80])
                        if ret:
                            with self.lock:
                                self.last_frame = jpeg_buffer.tobytes()
...
# Objeyi Başlatıyoruz
video_manager = VideoManager(udp_ip="0.0.0.0", udp_port=5000)

# --- Configuration & Paths ---
PYMAVLINK_PATH = r"./pymavlink_custom"
...
# --- VİDEO YAYINI (FRONTEND İÇİN) ---
def generate_video_frames():
    while True:
        frame = video_manager.get_frame()
        if frame is not None:
            # Görüntüyü HTTP stream formatına çevir
            yield (b'--frame\r\n'
...
async def video_feed():
    # Frontend'in bu adrese gelerek görüntüyü canlı izlemesini sağlar
