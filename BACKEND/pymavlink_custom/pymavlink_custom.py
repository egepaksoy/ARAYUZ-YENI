...
class Vehicle():
    ...
    def arm_disarm(self, arm: bool=True, drone_id: int=None, force_arm: bool=False):
        if drone_id is None:
            drone_id = self.drone_id
    ...
    def set_servo(self, channel: int=9, pwm: int=1000, drone_id: int=None):
        if drone_id is None:
            drone_id = self.drone_id
        ...
        try:
            ...
            if pwm > 2000 or pwm < 1000:
                Exception(f"Servo PWM degeri 1000-2000 deger araliginda olmali verilen deger aralik disinda: {pwm}")
    ...
    # Servonun pwm degerini okur
    def get_servo_pwm(self, channel: int, drone_id: int=None):
        if drone_id is None:
            drone_id = self.drone_id
        ...
        try:
            start_time = time.time()
            while not self.stop_event.is_set():
                message = self.vehicle.recv_match(type='SERVO_OUTPUT_RAW', blocking=True)
                
                if self.parse_message(message)[1] == drone_id:
                    return int(getattr(message, f"servo{channel}_raw"))

                if time.time() - start_time > 5:
                    print(f"UYARI 5 SANIYEDIR DRONE {drone_id} DEN SERVO DEGERI CEKILEMEDI!!!")
                    start_time = time.time()
    ...
    def on_location(self, loc, seq: int=0, sapma: int=1, drone_loc: list=None, drone_id: int=None):
        ...
        try:
            ...
            if seq != 0:
                ...
                if geodesic(drone_pos[:2], loc[:2]).meters <= sapma:
                    return True
                return False
        except Exception as e:
            return e
